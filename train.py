...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_seg2I.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_seg2I.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def get_date():
    # 返回当前时间，格式为20240101101010，包含年月日时分秒
    return time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))

def main():
    args = get_parser()
    os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(str(x) for x in args.train_gpu)
    # 文件拼接格式  save_path_runtime
    run_time = get_date()
    args.save_path = args.save_path + '_' + str(run_time)
